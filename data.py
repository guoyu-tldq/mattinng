...
class human_matting(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # _mask = np.where(_mask > 50, 1, 0).astype(np.uint8)
    ...
    def getFileName(self, idx):
        ...
        # 爱分割数据集
        nameIm = line.split('@')[0]
        nameTar = line.split('@')[3].split('\n')[0]

        #supervisely数据集
        # line = line.replace(' ', '@')
        # name = line.split('@')[0]
        # nameIm = os.path.join(self._base_dir, "img/", name)
        # name = line.split('@')[1].split('\n')[0]
        # nameTar = os.path.join(self._base_dir, "mask/", name)
        # print(nameIm, nameTar)
